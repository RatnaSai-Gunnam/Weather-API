...
def _normalize_row(row):
    ...
    max_temp = clean(row["max"], 10.0)          # tenths °C → °C
    min_temp = clean(row["min"], 10.0)          # tenths °C → °C
    precipitation = clean(row["precip"], 100.0) # tenths mm → cm
...
def ingest_weather_data(data_dir="wx_data"):
    ...
    for idx, file in enumerate(files, 1):
        ...
        for _, r in df.iterrows():
            record_date, max_temp, min_temp, precipitation = _normalize_row(r)
            records.append(
                dict(
